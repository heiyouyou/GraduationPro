...
module.exports = {
    ...
    // 格式化设备类型的helper方法
    formatType:function(val,options){
        if(val=='001')return "三星";
        if(val=='002')return "华为";
        if(val=='003')return "LG";
        if(val=='004')return "胜利";
        if(val=='005')return "长城";
    },
    //自定义标签方法,name:数据的key
